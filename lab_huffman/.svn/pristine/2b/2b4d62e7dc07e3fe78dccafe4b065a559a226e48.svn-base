...
HuffmanTree::TreeNode * HuffmanTree::removeSmallest( 
	...
	// your code!
	//TreeNode * sfront = singleQueue.front();
	//TreeNode * mfront = mergeQueue.front();
	//if(sfront==NULL&&mfront==NULL)
	//	return NULL;
...
void HuffmanTree::buildTree( const vector<Frequency> & frequencies ) {
	 ...
	 */
	 ...
	 while ((mergeQueue.size()+singleQueue.size())>1){
		...
		TreeNode* internal = new TreeNode(small->freq.getFrequency()+small2->freq.getFrequency());
