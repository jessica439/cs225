...
class BinaryTree
{
    ...
    private:
		...
		//private helper function mirrors all below
		void mirror(Node * subRoot);
		//private helper checks if below is ordered
		void isOrdered(const Node * subRoot, vector<T> & V)const;
		void printPaths(const Node* subRoot, vector<T> & temp)const;
		void printl(vector<T> temp)const;
		void sumDistances(const Node* subRoot, int & temp, int num)const;
};
