...
class KDTree
{
    ...
    private:
         ...
         */
 int partition(vector< Point<Dim> > & list, int left, int right, int pivotIndex, int dimen);
 Point<Dim> select(vector< Point<Dim> > & list, int left, int right, int k, int dimen);
void helper(vector< Point<Dim> > & newPoints, int left, int right, int dimen);
Point<Dim> findNearestNeighbor(int begin, int end, const Point<Dim> & query, int curDim) const;
int findDistance(const Point<Dim> p1, const Point<Dim> p2) const;
};
