...
PNG blendImages(PNG firstImage, PNG secondImage)
{
	...
	{
	    int firstRed = firstImage(xi,yi)->red;
	    int firstGreen = firstImage(xi,yi)->green;
	    int firstBlue = firstImage(xi,yi)->blue;
	    int secondRed = secondImage(xi,yi)->red;
	    int secondGreen = secondImage(xi,yi)->green;
	    int secondBlue = secondImage(xi,yi)->blue;
	    firstImage(xi,yi)->red = (firstRed+secondRed)/2;
	    firstImage(xi,yi)->green = (firstGreen+secondGreen)/2;
	    firstImage(xi,yi)->blue = (firstBlue+secondBlue)/2;
	}
    }
    return firstImage;
}
 ...
 */
namespace proxy_test { int __student_main(int argc, char ** argv); } int proxy_test::__student_main(int argc, char ** argv)
{
