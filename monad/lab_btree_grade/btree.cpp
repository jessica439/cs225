...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    * If first_larger_idx is a valid index and the key there is the key we
     * are looking for, we are done. *
	 //if(first_larger_idx ==key
	if(subroot->elements[insertion_idx(subroot->elements,key)].key==key)
		return subroot->elements[insertion_idx(subroot->elements,key)].value;
    * Otherwise, we need to figure out which child to explore. For this we
     ...
     *
	 	...
	 	if(subroot->is_leaf)
			return V();
	 	return find(subroot->children[insertion_idx(subroot->elements,key)],key);
	 }*/
    return V();
}
